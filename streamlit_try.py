...
# Function to load data from SQLite database into DataFrame
...
def main():
   ...
   # selected_table = st.sidebar.selectbox("Select a table", table_names)
    ...
    if st.button("Delete"):
        try:
            # Parse user inputs and convert to lists of integers and strings
    ...
    if st.button("Add/Update Entry"):
        try:
            conn = sqlite3.connect('/Users/aryasachar/Desktop/final_cleaned_project.db')
            df = load_data('accidents')  # Load the 'accidents' table data
            df = df.append(new_entry, ignore_index=True)
            df.to_sql('accidents', conn, if_exists='replace', index=False)
            conn.close()
            st.success("New entry added successfully!")

            # Refresh the DataFrame with the updated data
            df = load_data('accidents')
